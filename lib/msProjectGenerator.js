...
<Project xmlns="http://schemas.microsoft.com/project">
  ...
  xml += `
    <Task>
      ...
      <Finish>${formatDate(projectStart)}</Finish>
      <Summary>1</Summary>
      <ManualStart>${formatDate(projectStart)}</ManualStart>
      <ManualFinish>${formatDate(projectStart)}</ManualFinish>
    </Task>`;
...
function generateTaskXML(task, id, projectStart) {
  ...
  finishDate.setDate(finishDate.getDate() + parseInt(task.duration));
...
function parseTextWBS(text) {
  ...
  lines.forEach(line => {
    ...
    // Extract task info
    const match = trimmed.match(/^[\d.]*\s*(.+?)(?:\s*\((\d+)\s*days?\))?$/i);
    if (!match) return;

    const name = match[1].trim();
    const duration = match[2] ? parseInt(match[2]) : 1;

    const task = {
      name,
      duration,
      children: [],
      wbs: trimmed.match(/^[\d.]+/)?.[0] || ''
    };
