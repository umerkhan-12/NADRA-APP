...
import { assignQueuePosition } from "@/lib/queueHelper";
...
export async function assignTicketToAgent(ticket, agent) {
  ...
  // Queue email (non-blocking via Bull)
  queueEmail({
...
export async function createTicket({ serviceId, customerPriority, userId }) {
  ...
  // ✅ USE TRANSACTION to prevent race condition in agent assignment
  const ticket = await prisma.$transaction(async (tx) => {
    // Find available agent within transaction
    ...
    const newTicket = await tx.ticket.create({
      data: {
        ...
        servicePriority: service.defaultPriority || "MEDIUM",
        customerPriority,
        finalPriority,
  ...
  // Re-fetch ticket with payment to return complete data
  ...
  await assignQueuePosition(ticketWithPayment.id);
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <p><strong>Status:</strong> ${ticket.agent ? "Assigned" : "Waiting"}</p>
          ${ticket.agent ? `<p><strong>Assigned Agent:</strong> ${ticket.agent.name}</p>` : ""}
        </div>
